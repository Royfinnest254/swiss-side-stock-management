...
router.get('/items', async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM laundry_items WHERE is_active = 1 ORDER BY category, name');
    res.json(rows);
  } catch (err) {
    console.error(err);
...
router.delete('/items/:id', async (req, res) => {
  try {
    await pool.query('UPDATE laundry_items SET is_active = 0, deleted_by = ?, deleted_at = NOW() WHERE id = ?', [req.user.id, req.params.id]);
    res.json({ success: true });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Server error.' });
...
router.post('/withdraw', async (req, res) => {
  ...
  try {
    ...
    const [items] = await pool.query('SELECT quantity FROM laundry_items WHERE id = ? AND is_active = 1', [item_id]);
    if (!items.length) return res.status(404).json({ error: 'Item not found.' });
    if (parseFloat(items[0].quantity) < qty) return res.status(400).json({ error: 'Insufficient stock.' });

    await pool.query('UPDATE laundry_items SET quantity = quantity - ?, last_withdrawn_at = NOW() WHERE id = ? AND quantity >= ?', [qty, item_id, qty]);
    const [[{ remaining }]] = await pool.query('SELECT quantity as remaining FROM laundry_items WHERE id = ?', [item_id]);
    if (remaining === undefined) return res.status(404).json({ error: 'Item not found.' });
    await pool.query(
      'INSERT INTO laundry_transactions (item_id, action, quantity, transaction_date, reason, action_by) VALUES (?, "withdraw", ?, COALESCE(?, CURDATE()), ?, ?)',
    ...
    console.error('[Laundry Withdraw Error]', err.message);
    res.status(500).json({ error: 'Server error.' });
  }
});
...
router.post('/restock', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      'UPDATE laundry_items SET quantity = quantity + ?, last_restocked_at = NOW(), unit_price = COALESCE(?, unit_price) WHERE id = ?',
    ...
    console.error('[Laundry Restock Error]', err.message);
    res.status(500).json({ error: 'Server error.' });
  }
});
