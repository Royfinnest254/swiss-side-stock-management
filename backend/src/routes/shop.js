...
router.post('/items', async (req, res) => {
  ...
  try {
    const [result] = await pool.query(
      'INSERT INTO shop_items (name, category, quantity, unit, reorder_level, notes, is_folder, parent_id, classification, unit_price) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
...
router.put('/items/:id', async (req, res) => {
  ...
  try {
    await pool.query(
      'UPDATE shop_items SET name = ?, category = ?, unit = ?, reorder_level = ?, notes = ?, quantity = ?, is_folder = ?, parent_id = ?, classification = ?, unit_price = ? WHERE id = ?',
...
router.post('/withdraw', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      'INSERT INTO shop_transactions (item_id, action, quantity, transaction_date, reason, action_by) VALUES (?, "withdraw", ?, COALESCE(?, CURDATE()), ?, ?)',
