...
router.post('/items', async (req, res) => {
  ...
  try {
    const [result] = await pool.query(
      'INSERT INTO supplies_items (name, category, quantity, unit, reorder_level, notes, is_folder, parent_id, classification) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
...
router.put('/items/:id', async (req, res) => {
  ...
  try {
    await pool.query(
      'UPDATE supplies_items SET name = ?, category = ?, unit = ?, reorder_level = ?, notes = ?, quantity = ?, is_folder = ?, parent_id = ?, classification = ? WHERE id = ?',
...
router.post('/restock', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      'INSERT INTO supplies_transactions (item_id, action, quantity, transaction_date, reason, action_by) VALUES (?, "restock", ?, COALESCE(?, CURDATE()), ?, ?)',
