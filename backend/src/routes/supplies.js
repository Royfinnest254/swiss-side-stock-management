...
router.get('/items', async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM supplies_items WHERE is_active = 1 ORDER BY category, name');
    res.json(rows);
  } catch (err) {
    console.error(err);
...
router.post('/items', async (req, res) => {
  ...
  try {
    const [result] = await pool.query(
      'INSERT INTO supplies_items (name, category, quantity, unit, reorder_level, notes, is_folder, parent_id, classification, unit_price) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    ...
    await pool.query(
      'INSERT INTO supplies_transactions (item_id, action, quantity, transaction_date, action_by) VALUES (?, "added", ?, CURDATE(), ?)',
...
router.put('/items/:id', async (req, res) => {
  ...
  try {
    await pool.query(
      'UPDATE supplies_items SET name = ?, category = ?, unit = ?, reorder_level = ?, notes = ?, quantity = ?, is_folder = ?, parent_id = ?, classification = ?, unit_price = ? WHERE id = ?',
    ...
    await pool.query(
      'INSERT INTO supplies_transactions (item_id, action, quantity, transaction_date, action_by) VALUES (?, "edited", ?, CURDATE(), ?)',
...
router.delete('/items/:id', async (req, res) => {
  try {
    await pool.query('UPDATE supplies_items SET is_active = 0, deleted_by = ?, deleted_at = NOW() WHERE id = ?', [req.user.id, req.params.id]);
    res.json({ success: true });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Server error.' });
...
router.post('/withdraw', async (req, res) => {
  ...
  try {
    ...
    const [items] = await pool.query('SELECT quantity FROM supplies_items WHERE id = ? AND is_active = 1', [item_id]);
    if (!items.length) return res.status(404).json({ error: 'Item not found.' });
    if (parseFloat(items[0].quantity) < qty) return res.status(400).json({ error: 'Insufficient stock.' });

    await pool.query('UPDATE supplies_items SET quantity = quantity - ?, last_withdrawn_at = NOW() WHERE id = ? AND quantity >= ?', [qty, item_id, qty]);
    const [[{ remaining }]] = await pool.query('SELECT quantity as remaining FROM supplies_items WHERE id = ?', [item_id]);
    if (remaining === undefined) return res.status(404).json({ error: 'Item not found.' });
    await pool.query(
      'INSERT INTO supplies_transactions (item_id, action, quantity, transaction_date, reason, action_by) VALUES (?, "withdraw", ?, COALESCE(?, CURDATE()), ?, ?)',
      [item_id, qty, tDate, reason || null, req.user.id]
    );
    const [updated] = await pool.query('SELECT * FROM supplies_items WHERE id = ?', [item_id]);
    res.json({ success: true, item: updated[0] });
  } catch (err) {
    console.error('[Supplies Withdraw Error]', err.message);
    res.status(500).json({ error: 'Server error.' });
  }
});
...
router.post('/restock', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      'UPDATE supplies_items SET quantity = quantity + ?, last_restocked_at = NOW(), unit_price = COALESCE(?, unit_price) WHERE id = ?',
      [qty, unit_price_paid, item_id]
    );
    await pool.query(
      'INSERT INTO supplies_transactions (item_id, action, quantity, transaction_date, reason, action_by, unit_price_paid) VALUES (?, "restock", ?, COALESCE(?, CURDATE()), ?, ?, ?)',
      [item_id, qty, transaction_date || null, reason || null, req.user.id, unit_price_paid != null ? unit_price_paid : null]
    );
    const [updated] = await pool.query('SELECT * FROM supplies_items WHERE id = ?', [item_id]);
    res.json({ success: true, item: updated[0] });
  } catch (err) {
    console.error('[Supplies Restock Error]', err.message);
    res.status(500).json({ error: 'Server error.' });
  }
});
