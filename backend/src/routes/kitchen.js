...
router.post('/items', async (req, res) => {
  ...
  try {
    const [result] = await pool.query(
      'INSERT INTO kitchen_items (name, category, quantity, unit, reorder_level, notes, is_folder, parent_id, classification, unit_price) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [name, normalizedCategory, quantity || 0, unit || 'pcs', reorder_level || 0, notes || null, is_folder ? 1 : 0, parent_id || null, classification || null, unit_price != null ? unit_price : null]
    );
    await pool.query(
...
router.put('/items/:id', async (req, res) => {
  ...
  try {
    await pool.query(
      'UPDATE kitchen_items SET name = ?, category = ?, unit = ?, reorder_level = ?, notes = ?, quantity = ?, is_folder = ?, parent_id = ?, classification = ?, unit_price = ? WHERE id = ?',
      [name, normalizedCategory, unit || 'pcs', reorder_level || 0, notes || null, quantity || 0, is_folder ? 1 : 0, parent_id || null, classification || null, unit_price != null ? unit_price : null, req.params.id]
    );
    await pool.query(
...
router.post('/restock', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      'UPDATE kitchen_items SET quantity = quantity + ?, last_restocked_at = NOW(), unit_price = COALESCE(?, unit_price) WHERE id = ?',
      [qty, unit_price_paid, item_id]
    );
    await pool.query(
      'INSERT INTO kitchen_transactions (item_id, action, quantity, transaction_date, reason, action_by, unit_price_paid) VALUES (?, "restock", ?, COALESCE(?, CURDATE()), ?, ?, ?)',
