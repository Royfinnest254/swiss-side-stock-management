...
import { Flower2, Plus, Search, Loader2, Package, ArrowUpRight, ArrowDownLeft, Trash2, Clock, History, CheckCircle2, AlertCircle, Edit2, Wrench, Calendar, Filter, Folder } from 'lucide-react';
...
export default function Spa() {
  ...
  const fetchData = async () => {
    ...
    try {
      if (activeTab === 'list') {
        const data = await api.get('/spa/items');
        setItems(data.results || data);
      } else if (activeTab === 'history') {
        let endpoint = '/spa/transactions';
        const params = [];
        ...
        const data = await api.get('/spa/maintenance');
        setMaintenance(data.results || data);
        // Refresh items for log maintenance dropdown
        const listData = await api.get('/spa/items');
        setItems(listData.results || listData);
      }
    } catch (err) { 
      toast.error('Failed to update spa records'); 
    } finally { 
      setLoading(false); 
    }
  };
  ...
  const handleSaveItem = async (e) => {
    ...
    const payload = {
      ...
      category: itemForm.is_folder ? 'Product' : itemForm.category,
    ...
    try {
      if (itemModal.mode === 'add') {
        await api.post('/spa/items', payload);
        toast.success('Item added to records');
      } else {
        await api.put(`/spa/items/${itemModal.data.id}`, payload);
  ...
  const handleStockUpdate = async (e) => {
    ...
    const endpoint = isRestock ? '/spa/restock' : '/spa/withdraw';
    try {
  ...
  const handleMaintSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await api.post('/spa/maintenance', {
  ...
  const handleResolveMaintenance = async (e) => {
    ...
    setSubmitting(true);
    try {
      await api.patch(`/spa/maintenance/${resolveModal.data.id}/resolve`, {
  ...
  const handleDismissMaintenance = async (id) => {
    ...
    setSubmitting(true);
    try {
      await api.patch(`/spa/maintenance/${id}/dismiss`);
  ...
  const handleDelete = async () => {
    setSubmitting(true);
    try {
      await api.delete(`/spa/items/${deleteModal.data.id}`);
    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-[#F3F4F6]">
        <div>
          ...
          <h1 className="text-3xl font-black text-[#1A1A1A] tracking-tight uppercase">Spa</h1>
        ...
        {activeTab === 'list' && (
          <button onClick={() => {
            setItemForm({ name: '', category: 'Product', quantity: 0, unit: 'pcs', reorder_level: 5, notes: '', is_folder: false, parent_id: null, classification: '', unit_price: '' });
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-4">
                            <div className="w-10 h-10 bg-[#FDF5F3] text-[#A0604E] rounded-xl flex items-center justify-center shrink-0">
                              {item.is_folder ? <Folder size={20} /> : <Flower2 size={20} />}
                            </div>
                            <div>
        ...
        <form onSubmit={handleSaveItem} className="space-y-6">
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-1.5 col-span-2">
              ...
              <input className="input-field" value={itemForm.name} onChange={e => setItemForm({...itemForm, name: e.target.value})} required placeholder="e.g. Spa Oil" />
            </div>
            ...
            {!itemForm.is_folder && (
              <>
                ...
                <div className="space-y-1.5">
                  ...
                  <input className="input-field" value={itemForm.unit} onChange={e => setItemForm({...itemForm, unit: e.target.value})} required placeholder="pcs, ml, liters" />
                </div>
          ...
          <button type="submit" disabled={submitting} className="btn-primary w-full h-14 uppercase tracking-widest font-black mt-6">Confirm</button>
        </form>
      </Modal>
