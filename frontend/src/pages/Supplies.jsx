...
import { Truck, Plus, Search, Loader2, Package, ArrowUpRight, ArrowDownLeft, Trash2, Clock, History, CheckCircle2, AlertCircle, Wrench, Calendar, Filter, Folder } from 'lucide-react';
...
export default function Supplies() {
  ...
  // Registration form: accepts name, quantity, unit, reorder_level, status (condition), notes, type (Product or Equipment)
  const [itemForm, setItemForm] = useState({ 
    name: '', 
    type: 'Products & Supplies', // Matches 'Products & Supplies' or 'Equipment'
    quantity: 0, 
    unit: 'pcs', 
    reorder_level: 5, 
    status: 'good', // condition status
    notes: '',
    is_folder: false,
    parent_id: null,
    classification: '',
  ...
  // Maintenance states matching Gym / Spa / Kitchen / Laundry / Shop
  ...
  const fetchData = async () => {
    ...
    try {
      if (activeTab === 'list') {
        const data = await api.get('/supplies/items');
        setItems(data.results || data);
      } else if (activeTab === 'history') {
        let endpoint = '/supplies/transactions';
        const params = [];
        ...
        // Fetch from global needs endpoint filtered by department = Supplies and type = Maintenance
        const data = await api.get('/needs');
        const list = data.results || data;
        setMaintenance(list.filter(n => n.department === 'Supplies' && n.request_type === 'Maintenance'));
        // Refresh items for dropdown
        const itemsData = await api.get('/supplies/items');
        setItems(itemsData.results || itemsData);
      }
    } catch (err) { 
      toast.error('Failed to update supplies records'); 
    } finally { 
      setLoading(false); 
    }
  };
  ...
  const handleSaveItem = async (e) => {
    ...
    try {
      if (itemModal.mode === 'add') {
        await api.post('/supplies/items', payload);
        toast.success('Item added to records');
      } else {
        await api.put(`/supplies/items/${itemModal.data.id}`, payload);
  ...
  const handleStockUpdate = async (e) => {
    ...
    const endpoint = isRestock ? '/supplies/restock' : '/supplies/withdraw';
    try {
  ...
  const handleDelete = async () => {
    setSubmitting(true);
    try {
      await api.delete(`/supplies/items/${deleteModal.data.id}`);
    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-[#F3F4F6]">
        <div>
          ...
          <h1 className="text-3xl font-black text-[#1A1A1A] tracking-tight uppercase">Supplies</h1>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-4">
                            <div className="w-10 h-10 bg-[#FDF5F3] text-[#A0604E] rounded-xl flex items-center justify-center shrink-0">
                              {item.is_folder ? <Folder size={20} /> : <Truck size={20} />}
                            </div>
                            <div>
      ...
      <Modal isOpen={itemModal.open} onClose={() => setItemModal({ ...itemModal, open: false })} title={itemModal.mode === 'add' ? 'Register Item' : 'Modify Item'}>
        <form onSubmit={handleSaveItem} className="space-y-6">
          ...
          <div className="space-y-1.5">
            ...
            <input className="input-field" value={itemForm.name} onChange={e => setItemForm({...itemForm, name: e.target.value})} required placeholder="e.g. Toilet Paper rolls" />
          </div>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-1.5">
                  ...
                  <input className="input-field" value={itemForm.unit} onChange={e => setItemForm({...itemForm, unit: e.target.value})} required placeholder="pcs, packs" />
                </div>
