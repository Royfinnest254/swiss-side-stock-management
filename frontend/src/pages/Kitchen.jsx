...
export default function Kitchen() {
  ...
  const handleDismissMaintenance = async (id) => {
    ...
    setSubmitting(true);
    try {
      await api.patch(`/kitchen/maintenance/${id}/dismiss`);
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-[#F3F4F6]">
        ...
        {activeTab === 'list' && (
          <button onClick={() => {
            setItemForm({ name: '', quantity: 0, unit: 'pcs', reorder_level: 5, category: 'Consumables', notes: '', is_folder: false, parent_id: null, classification: '', unit_price: '' });
