...
import { WashingMachine, Plus, Search, Loader2, Package, ArrowUpRight, ArrowDownLeft, Trash2, Clock, History, CheckCircle2, AlertCircle, Wrench, Calendar, Filter, Folder } from 'lucide-react';
...
export default function Laundry() {
  ...
  // Maintenance states matching Gym / Spa / Kitchen
  ...
  const fetchData = async () => {
    ...
    try {
      if (activeTab === 'list') {
        const data = await api.get('/laundry/items');
        setItems(data.results || data);
      } else if (activeTab === 'history') {
        let endpoint = '/laundry/transactions';
        const params = [];
        ...
        // Fetch from global needs endpoint filtered by department = Laundry and type = Maintenance
        const data = await api.get('/needs');
        const list = data.results || data;
        setMaintenance(list.filter(n => n.department === 'Laundry' && n.request_type === 'Maintenance'));
        // Refresh items for dropdown
        const itemsData = await api.get('/laundry/items');
        setItems(itemsData.results || itemsData);
      }
    } catch (err) { 
      toast.error('Failed to update laundry records'); 
    } finally { 
      setLoading(false); 
    }
  };
  ...
  const handleBulkMaintenance = async (e) => {
    ...
    setSubmitting(true);
    try {
      await api.post('/laundry/maintenance/bulk', {
  ...
  const handleSaveItem = async (e) => {
    ...
    const payload = {
      ...
      reorder_level: itemForm.is_folder ? 0 : (itemForm.type === 'Products & Supplies' ? itemForm.reorder_level : 0),
  ...
  const handleStockUpdate = async (e) => {
    ...
    const endpoint = isRestock ? '/laundry/restock' : '/laundry/withdraw';
    try {
  ...
  const handleDelete = async () => {
    setSubmitting(true);
    try {
      await api.delete(`/laundry/items/${deleteModal.data.id}`);
    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-[#F3F4F6]">
        <div>
          ...
          <h1 className="text-3xl font-black text-[#1A1A1A] tracking-tight uppercase">Laundry</h1>
      ...
      {activeTab === 'list' ? (
        <div className="space-y-6">
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
            ...
            <div className="flex items-center gap-1 bg-[#F9FAFB] rounded-xl p-1 w-fit border border-[#F3F4F6] shrink-0">
              {[
                { id: 'all', label: 'All' },
                ...
                <button
                  ...
                  type="button"
                  className={`text-[10px] font-black uppercase tracking-widest px-4 h-9 rounded-lg transition-all ${
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-4">
                            <div className="w-10 h-10 bg-[#FDF5F3] text-[#A0604E] rounded-xl flex items-center justify-center shrink-0">
                              {item.is_folder ? <Folder size={20} /> : <WashingMachine size={20} />}
                            </div>
                            <div>
                        ...
                        <td className="hidden md:table-cell px-6 py-4">
                          {item.is_folder ? (
                            ...
                            <div className="flex items-baseline gap-1">
                              ...
                              <span className="text-sm font-black text-[#1A1A1A]">{item.unit_price != null && item.unit_price !== '' ? parseFloat(item.unit_price).toFixed(2) : '—'}</span>
                            </div>
                          )}
                        </td>
                          ...
                          {item.is_folder ? (
                            ...
                            <div className="flex items-baseline gap-1">
                              ...
                              <span className="text-sm font-black text-[#1A1A1A]">
                                {item.unit_price != null && item.unit_price !== '' ? (parseFloat(item.unit_price) * parseFloat(item.quantity || 0)).toFixed(2) : '—'}
                              </span>
                            </div>
                          )}
                        </td>
        ...
        <form onSubmit={handleStockUpdate} className="space-y-6 py-4">
          ...
          <div className="space-y-4">
            ...
            {stockModal.type === 'restock' && (
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-[#9CA3AF] uppercase tracking-widest ml-1">Unit Cost Price (KES) <span className="text-[9px] font-normal normal-case text-[#9CA3AF]">— optional</span></label>
                <input
                  type="number"
                  step="0.01"
                  min="0"
      ...
      <Modal isOpen={itemModal.open} onClose={() => setItemModal({ ...itemModal, open: false })} title={itemModal.mode === 'add' ? 'Register Item' : 'Modify Item'}>
        <form onSubmit={handleSaveItem} className="space-y-6">
          ...
          <div className="space-y-1.5">
            ...
            <input className="input-field" value={itemForm.name} onChange={e => setItemForm({...itemForm, name: e.target.value})} required placeholder="e.g. Ironing Board" />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <select className="input-field" value={itemForm.type} onChange={e => setItemForm({...itemForm, type: e.target.value})}>
              <option value="Products & Supplies">Products & Supplies</option>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-1.5">
                  ...
                  <input className="input-field" value={itemForm.unit} onChange={e => setItemForm({...itemForm, unit: e.target.value})} required placeholder="pcs, liters, kg" />
                </div>
