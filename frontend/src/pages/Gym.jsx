...
import { Dumbbell, Plus, Search, Loader2, Package, ArrowUpRight, ArrowDownLeft, Trash2, Clock, History, CheckCircle2, AlertCircle, Wrench, Calendar, Filter, Folder } from 'lucide-react';
...
export default function Gym() {
  ...
  const fetchData = async () => {
    ...
    try {
      if (activeTab === 'list') {
        const data = await api.get('/gym/inventory');
        setItems(data.results || data);
      } else if (activeTab === 'history') {
        let endpoint = '/gym/transactions';
        const params = [];
        ...
        const data = await api.get('/gym/maintenance');
        setMaintenance(data.results || data);
        // Refresh items for maintenance dropdown
        const listData = await api.get('/gym/inventory');
        setItems(listData.results || listData);
      }
    } catch (err) { 
      toast.error('Failed to update gym records'); 
    } finally { 
      setLoading(false); 
    }
  };
  ...
  const handleSaveItem = async (e) => {
    ...
    // Normalize values based on selected type choice (Product vs Equipment) and Folders
    const payload = {
      name: itemForm.name,
      quantity: itemForm.is_folder ? 0 : itemForm.quantity,
      unit: itemForm.is_folder ? 'folder' : itemForm.unit,
      reorder_level: (itemForm.type === 'Product' && !itemForm.is_folder) ? itemForm.reorder_level : 0,
      status: itemForm.type === 'Equipment' ? itemForm.status : 'ok',
    ...
    try {
      if (itemModal.mode === 'add') {
        await api.post('/gym/inventory', payload);
        toast.success('Item registered successfully');
      } else {
        await api.put(`/gym/inventory/${itemModal.data.id}`, payload);
  ...
  const handleMaintSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await api.post('/gym/maintenance', {
  ...
  const handleResolveMaintenance = async (e) => {
    ...
    setSubmitting(true);
    try {
      await api.patch(`/gym/maintenance/${resolveModal.data.id}/resolve`, {
  ...
  const handleDismissMaintenance = async (id) => {
    ...
    setSubmitting(true);
    try {
      await api.patch(`/gym/maintenance/${id}/dismiss`);
  ...
  const handleDelete = async () => {
    setSubmitting(true);
    try {
      await api.delete(`/gym/inventory/${deleteModal.data.id}`);
      toast.success('Equipment removed from records');
  ...
  const getStatus = (item) => {
    ...
    if (item.status === 'repair_needed' || item.status === 'broken') return 'broken';
  ...
  const filtered = items.filter(i => {
    ...
    const isEquip = i.notes?.includes('[Equipment]') || i.category === 'Equipment';
    const isProd = i.notes?.includes('[Product]') || i.category === 'Product' || (!isEquip && !i.category);
    if (filterClassification === 'equipment') return matchesSearch && isEquip;
    if (filterClassification === 'product') return matchesSearch && isProd;
    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-[#F3F4F6]">
        <div>
          ...
          <h1 className="text-3xl font-black text-[#1A1A1A] tracking-tight uppercase">Gym</h1>
      ...
      {activeTab === 'list' ? (
        <div className="space-y-6">
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
            <div className="flex items-center gap-3 bg-white border border-[#F3F4F6] rounded-2xl px-6 py-1 w-full max-w-md shadow-sm">
            ...
            <div className="flex bg-gray-100 p-1 rounded-xl shrink-0">
              {[
                ...
                { id: 'product', label: 'Products / Supplies' }
              ].map(opt => (
                <button
                  key={opt.id}
                  onClick={() => setFilterClassification(opt.id)}
                  type="button"
                  className={`text-[10px] font-black uppercase tracking-wider px-4 py-2 rounded-lg transition-all ${
                    filterClassification === opt.id 
                      ? 'bg-[#A0604E] text-white shadow-sm' 
                      : 'text-gray-500 hover:text-[#A0604E]'
                  }`}
                >
                  {opt.label}
                </button>
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <span className="text-[10px] font-black text-[#9CA3AF] uppercase tracking-widest block mt-0.5">
                                ...
                                {item.classification ? `🏷️ ${item.classification}  •  ` : ''}
                                {item.notes ? item.notes.replace('[Equipment]', '').replace('[Product]', '').trim() : 'No notes'}
                              </span>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="hidden md:table-cell px-6 py-4">
                          {item.is_folder ? (
                            ...
                            (() => {
                              ...
                              if (status === 'broken') return <span className="px-3 py-1 rounded-lg bg-[#FCEBEB] text-[#A32D2D] text-[9px] font-black uppercase tracking-[0.15em]">Repair Needed</span>;
                        ...
                        <td className="text-right px-6 py-4">
                          <div className="flex justify-end gap-2">
                            {item.is_folder ? (
                              <button 
                                ...
                                onClick={() => {
                                  setItemForm({
                                    name: '',
                                    type: item.notes?.includes('[Equipment]') ? 'Equipment' : 'Product',
                                    quantity: 0,
                                    unit: 'pcs',
                                    reorder_level: 5,
                                    notes: '',
                            ...
                            <button title="Edit Item" onClick={() => {
                              const isEquip = item.notes?.includes('[Equipment]');
                              setItemForm({ 
                                name: item.name, 
                                type: isEquip ? 'Equipment' : 'Product',
        ...
        <form onSubmit={handleSaveItem} className="space-y-6">
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-1.5 col-span-2">
              ...
              <input className="input-field" value={itemForm.name} onChange={e => setItemForm({...itemForm, name: e.target.value})} required placeholder="e.g. Spin Bike" />
            </div>
            ...
            <div className="space-y-1.5 col-span-2 flex items-center gap-3 bg-gray-50 p-4 rounded-2xl border border-gray-100/80">
              <input 
                ...
                onChange={e => setItemForm({
                  ...
                  // If it's a folder, it cannot have a parent folder
              ...
              <>
                ...
                <div className="space-y-1.5 col-span-2">
                  ...
                  <input 
                    ...
                    onChange={e => setItemForm({...itemForm, classification: e.target.value})} 
                    placeholder="e.g. Cardiorespiratory, Strength, Free Weights" 
                  />
                </div>

                <div className="space-y-1.5 col-span-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-[#9CA3AF] ml-1">Item Classification</label>
                ...
                <div className="space-y-1.5">
                  ...
                  <input className="input-field" value={itemForm.unit} onChange={e => setItemForm({...itemForm, unit: e.target.value})} required placeholder="pcs, sets" />
                </div>
